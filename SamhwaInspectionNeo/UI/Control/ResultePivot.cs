...
{
    public partial class ResultePivot : XtraUserControl
    {
        public ResultePivot() => InitializeComponent();
        ...
        public void Init()
        {
            ...
            foreach (검사그룹 그룹 in typeof(검사그룹).GetEnumValues())
            {
                ...
                GridBand band = new GridBand() { Caption = MvUtils.Utils.GetDescription(그룹), VisibleIndex = this.GridView1.Bands.Count };
            ...
            foreach (장치구분 장치 in typeof(장치구분).GetEnumValues())
            {
                ...
                GridBand band = new GridBand() { Caption = MvUtils.Utils.GetDescription(장치), VisibleIndex = this.GridView1.Bands.Count };
                band.AppearanceHeader.TextOptions.HAlignment = HorzAlignment.Center;
                band.AppearanceHeader.Options.UseTextOptions = true;
                this.GridView1.Bands.Add(band);
            }
        ...
        private void 검사결과보기(object sender, EventArgs e)
        {
            ...
            //Int32 코드 = MvUtils.Utils.IntValue(row[nameof(검사결과.검사코드)]);
