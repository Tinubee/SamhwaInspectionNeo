...
namespace SamhwaInspectionNeo.Schemas.Reports
{
    public class 검사내역 : IDisposable
    {
        ...
        public void Init(모델구분 모델)
        {
            ...
            foreach (검사정보 설정 in Global.모델자료.GetItem(모델구분).검사설정)
            {
                ...
                String toolTip = $"{col.Caption}.{MvUtils.Utils.GetDescription(설정.검사장치)}.{설정.검사항목} {MvUtils.Utils.GetDescription(설정.검사그룹)}({MvUtils.Utils.GetDescription(설정.측정단위)})";
        ...
        public String GetBand(장치구분 장치, 검사그룹 그룹) =>
            장치 == 장치구분.None ? MvUtils.Utils.GetDescription(그룹) : MvUtils.Utils.GetDescription(장치);
